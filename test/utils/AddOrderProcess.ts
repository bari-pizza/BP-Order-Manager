...
import { OrdersPage } from '../pages/OrdersPage/OrdersPageDesktop';
import { OrdersPageMobile } from '../pages/OrdersPage/OrdersPageMobile';

interface AddOrderProcessBaseOptions {
    isMobile?: boolean;
}

interface AddOrderProcessMobileOptions extends AddOrderProcessBaseOptions {
    isMobile: true;
    page: OrdersPageMobile;
}

interface AddOrderProcessDesktopOptions extends AddOrderProcessBaseOptions {
    isMobile: false;
    page: OrdersPage;
}

type AddOrderProcessOptions = AddOrderProcessMobileOptions | AddOrderProcessDesktopOptions;

export class AddOrderProcess {
    private ordersPage: OrdersPage;
    private ordersPageMobile: OrdersPageMobile;
    private page: OrdersPage | OrdersPageMobile;
    private static currentOrderNumber = 0;
    private static generatedNames: Set<string> = new Set();
    private isMobile: boolean;

    constructor({ page, isMobile }: AddOrderProcessOptions) {
        if (isMobile) {
            this.ordersPageMobile = page;
        } else {
    ...
    async startAddOrderProcess() {
        if (this.isMobile) {
            await this.ordersPageMobile.clickAddOrder();
        } else {
            await this.ordersPage.locator('button.MuiButton-contained:has-text("Add Order")').click();
        }
    }
